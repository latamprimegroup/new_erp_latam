 ...
 * Reserva itens do estoque para o produtor logado (atribuição exclusiva)
...
export async function POST(req: Request) {
  ...
  const limited = withRateLimit(req, getAuthenticatedKey(session.user!.id, 'estoque:reservar'), { max: 30, windowMs: 60_000 })
  ...
  try {
    ...
    let result
    if (tipo === 'email') {
      result = await reserveEmail(id, producerId)
    } else if (tipo === 'cnpj') {
      result = await reserveCnpj(id, producerId)
    } else if (tipo === 'perfil') {
      result = await reservePaymentProfile(id, producerId)
    } else {
