 ...
 *
 * Envia evento de venda confirmada com dados do comprador e UTMs
 * para o dashboard da Utmify calcular ROI por campanha.
 *
...
export async function sendUtmifyConversion(params: {
  ...
  if (!token) {
    console.warn('[Utmify] UTMIFY_API_TOKEN não configurado — ignorando envio')
    return false
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[Utmify] Falha ao enviar conversão (${res.status}):`, txt)
      return false
    }

    console.log(`[Utmify] Conversão enviada com sucesso — orderId: ${params.checkoutId}`)
    return true
  } catch (err) {
